...
# Round 4 — 数据集校验 (per-province 漏掉的手写数据集)

per-province pass 没碰的几个大数据集，5 个 agent 分头联网核查官方来源。

## ✅ 已修 (客观、无字段耦合)
- **city-tier-2025.json**: 厦门/大连/济南/昆明 由"新一线"改"二线"(第一财经 2024/2025 新一线 15 城名单均不含这 4 城; 原文件标 16 个新一线本身结构不可能); 厦门 schools_shuangyiliu 移除误挂的**福州大学**(福州大学在福州, 非厦门)。
- **mistake-zone-2025.json**: ①公安体测长跑 1500m→**男1000m/女800m**(1500m 是军校标准, 公安体测长跑是 1000/800); ②高水平运动队 2024 改革文化线 "一本/特控"→**普通类本科线**(双一流达本科线/其他校80%)——原表高估门槛, 可能误导达本科线的运动员弃报; ③港中深 11.5万→**14万/年**(2025 入学; 11.5 是 2022-24); ④"北师大珠海(转公办)"→**终止办学**(分校 2024 终止, 非转设)。
- **family-budget-2025.json**: income_distribution_reference 的 _source 把"人均可支配收入"误标成"家庭年收入分位", 且 p95/p99 在统计局源中根本不存在 → 改注为"样本估算·非统计局原值", 数额保留。

## ⏸️ 已核实但**未改** (字段耦合, 需整体重算, 避免改坏)
- **employment-outcomes-2024.json**: agent 用官方就业报告查出多校 升学/出国率系统性偏低 (电子科大 abroad 12→22, 北邮 abroad 9→14.88/domestic 45→50.73/保研 18→30, 华科 domestic 55→66, 西交 52→58.4 等)。**但每校 domestic+abroad+direct≈100 是内部平衡的**(电子科大现 55+12+28=95, 单改 abroad→22 会变 105 不可能), 必须按官方报告**整条重构**深造/就业拆分, 不能单字段改。已列清单, 留作专项人工修。另: 复旦/中科大/清华/西电/浙大 因本科vs全校口径歧义, agent 标 suspect。
- **kaogong-majors-2025.json**: post_categories 里 **税务局 posts_pct=24 实为把 24008 *人* 误当 24%**(2024 国考税务系统占岗位约 53%/招录人数约 61%, 是最大单一系统)。但该百分比序列凑成 100, 单改一项破坏总和, 需整组按真实职位表重算; 已标记。专业层面(法学/会计/计算机/财政学 等为招录大户)方向正确, 不报。

## 📌 数据缺口 (不杜撰)
- **西藏 一分一段表**: 31 省里唯一完全缺失 (老高考, 文理), 导致 `rank` 对西藏失败。一分一段是数百行的考试院 PDF 表, 无法靠联网搜索如实转录 → **不杜撰**, 标记为待人工从西藏考试院补 JSON (README 已有补录流程)。
- city-tier 新一线集合还差 东莞/宁波/无锡(2024)或佛山(2025) 未在表内 — 属补数据, 需核对后增补, 暂未加。

*方法同前: 每条 error 须有官方来源 URL 矛盾才落地; 有字段耦合/口径歧义的一律只标记不改。*

---

# Round 5 — employment-outcomes 深度重构 (45 top 校官方报告核对)

round 4 把 employment 标为"字段耦合需整体重算"。本轮 5 个 agent 逐校联网核对 **2024届本科就业质量报告**, 把 45 所 top 校(985+关键财经/邮电/政法)的 `baoyan/domestic/abroad/direct` 整条重构。

**系统性错误根因**: 多校 `domestic_grad_school_pct`(国内升学)字段被填成了**总深造率(含出国)**, 导致 domestic 虚高、abroad 虚低。例: 湖南大学原 domestic=48 实为总深造 47.12%, 真·境内升学=38。

**已修 27 校**(整条替换, 全部满足 domestic+abroad+direct≤100 且 domestic≥baoyan; 256 校平衡校验通过):
- 高可信(官方明示本科数): 北大 62/17/21→66/18/10、复旦 abroad 25→19、上交、中科大 domestic 70→66、哈工大(按人数核算 保研30→42/domestic 55→64)、天大(保研26→35/出国12→7)、湖南(domestic 48→38 纠"总深造"混淆)、电子科大(出国12→23/domestic 55→49)、西农、北邮(domestic 45→51/出国9→15)、中国政法(全字段错, 保研17→28/direct 25→52)。
- 中可信(权威转载/需小推算): 北航(direct 28→4, 该校深造率极高)、北理工、中国农大(domestic 60→45)、北师大、南开、同济、中南、重庆、中央财经(出国28→21)、西电、华东师大、南科大(境外41>境内38)、上财、西南财。

**未改 18 校**(官方只给总深造率/图片PDF/反爬/仅旧年份 → 保留现值, 标 low):
清华(仅官方出国18.4%, 无法配平整条)、浙大、南大、人大、东北大学、东南、厦门、山东、中国海洋、武汉、华科、中山、华南理工、四川、西北工大、兰州、对外经贸、大连理工(差<3pp)。

**仍未处理**: kaogong 税务局 posts_pct(序列凑100需整组重算); 256 校里的 ~200 所省属/民办(estimate 为主, `_caveat` 已声明)。

*方法: 字段口径严格对齐(domestic=境内升学含保研; abroad=出国境; direct=协议就业); 整条替换保平衡; 无官方来源一律 found=false 保留现值, 绝不杜撰。*
